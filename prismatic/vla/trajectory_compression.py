...
class BSplineTrajectoryCompressionV3(BaseTrajectoryCompression):
    def __init__(self):
        ...
        compression_json = Path(__file__).parent.parent.parent / "assets" / "compression_results.json"
    ...
    def get_cp_from_episode(self, episode_index: int):
        ...
        if episode_key not in self.episode_cache:
            raise ValueError(f"Episode index {episode_index} not found in compression results.")
        
        episode_data = self.episode_cache[episode_key]
        bspline_data = episode_data["bspline"]
        control_points = np.array(bspline_data["control_points"])
        knot_vectors = np.array(bspline_data["knot_vectors"][0]) # 取第一个维度的knot vector
        return control_points, knot_vectors
    ...
    def __call__(self, frame_index, episode_index) -> np.ndarray:
        ...
        使用固定数量的内部节点进行B样条最小二乘拟合，然后slice到后续的实际的trajectory
        """        
        contrlol_points, knot_vectors = self.get_cp_from_episode(episode_index)
        full_traj = np.hstack([contrlol_points, knot_vectors[3:-1]]) # knot vector去掉前3个和后一个，这样实现长度一致（能contact）
        
        idx = np.searchsorted(full_traj[:,-1], frame_index)
        slice_idx = max(0, idx - 3) # NOTE: 我们设定需要拿到-3的点开始预测, 维持i后的轨迹一致
    ...
    def decode_to_action(
        ...
        # gripper 从时刻 0 到 internal_knot_vector 结尾，0 阶插值
        t_eval = np.arange(0, int(internal_knot_vector[-1]) + 1)
        gripper_cp = control_points[:, 6]
        indices = np.searchsorted(internal_knot_vector, t_eval, side='right') - 1
        indices = np.clip(indices, 0, len(gripper_cp) - 1)
        gripper_traj = gripper_cp[indices]
